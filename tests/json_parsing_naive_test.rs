...
mod input {
    use super::{assert_err_eq, JsonParsingError};
...
mod literals {
    use super::{assert_err_eq, assert_ok_eq, JsonParsingError, JsonValue};
...
mod strings {
    ...
    mod pos {
        use super::{assert_ok_eq, char_from_hex, JsonValue};
        ...
        fn arrays_malformed_should_fail() {
            ...
            for case in cases {
                ...
                assert!(
                    matches!(
                        res,
                        Err(JsonParsingError::InvalidArray) | Err(JsonParsingError::InvalidJsonFile)
                    ),
                    "input was: {:?}, got: {:?}",
                    case,
                    res
    ...
    mod pos {
        ...
        fn object_simple_ok() {
            ...
            let cases = [
                ...
                (r#"{ "a" : 1 }"#, expected_one),
                (
                    r#"{ "a":1, "b":true, "c":null, "d":"hé", "e":[1,2] }"#,
                    expected_mixed,
                ),
            ];
